...
function gplot{T<:Real}(
    ...
    nodestrokec = nothing,
    nodestrokelw = 0.0,
    arrowlengthfrac = LightGraphs.is_directed(G) ? 0.1 : 0.0,
    arrowangleoffset = π/9.0,
    linetype = "straight",
    outangle = pi/5)
    ...
    if edgelabel != nothing
        ...
        for e in LightGraphs.edges(G)
            i = src(e)
            j = dst(e)
            push!(edge_locs_x, (locs_x[i]+locs_x[j])/2.0 + edgelabeldistx*NODESIZE)
            push!(edge_locs_y, (locs_y[i]+locs_y[j])/2.0 + edgelabeldisty*NODESIZE)
        end
        edgetexts = text(edge_locs_x, edge_locs_y, map(string, edgelabel), [hcenter], [vcenter])
    end
